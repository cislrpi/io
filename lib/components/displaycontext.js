...
module.exports = function () {
    function DisplayContext(name, window_settings, io) {
        ...
        this.io.onTopic("display.removed", function (m) {
            ...
            //clear up the store
            _this.io.store.getHash("display:dc:" + _this.name).then(function (m) {
                if (!_.isEmpty(m)) {
                    ...
                    if (mobj) {
                        _this.io.store.addToHash("display:dc:" + _this.name, "displayWinObjMap", JSON.stringify(mobj));
                    } else {
                        _this.io.store.removeFromHash("display:dc:" + _this.name, "displayWinObjMap");
                    }
                    ...
                    if (vobj) {
                        _this.io.store.addToHash("display:dc:" + _this.name, "viewObjDisplayMap", JSON.stringify(vobj));
                    } else {
                        _this.io.store.removeFromHash("display:dc:" + _this.name, "viewObjDisplayMap");
                    }
                }
            });
        });
    ...
    _createClass(DisplayContext, [{
        ...
        value: function restoreFromStore() {
            ...
            return this.io.store.getHash("display:dc:" + this.name).then(function (m) {
        ...
        value: function getWindowBounds() {
            ...
            return this.io.store.getHashField("display:windowBounds", this.name).then(function (m) {
                console.log("display:windowBounds", m);
                if (m == null) {
                    console.log("using display:displays for windowBounds");
                    return _this3.io.store.getHash("display:displays").then(function (x) {
        ...
        value: function show() {
            ...
            return this.getWindowBounds().then(function (m) {
                ...
                _this4.io.store.setState("display:activeDisplayContext", _this4.name);
                return m;
            });
        }
    }, {
        ...
        value: function close() {
            ...
            return this.getWindowBounds().then(function (m) {
                ...
                if (!isHidden) {
                    ...
                    _this6.io.store.delState('display:dc:' + _this6.name);
                    _this6.io.store.removeFromSet("display:displayContexts", _this6.name);
                    _this6.io.store.removeFromHash("display:windowBounds", _this6.name);
                    _this6.io.store.getState('display:activeDisplayContext').then(function (x) {
                        if (x == _this6.name) _this6.io.store.delState('display:activeDisplayContext');
                    });
                }
                return map;
            });
        ...
        */
        ...
        value: function initialize(options) {
            ...
            return this.show().then(function () {
                ...
                _this7.io.store.addToHash("display:dc:" + _this7.name, "displayWinObjMap", JSON.stringify(map));
                return map;
            });
        }
    }, {
        ...
        value: function createViewObject(options) {
            ...
            if (this.displayWindows.has(windowName)) {
                return this.displayWindows.get(windowName).createViewObject(options).then(function (vo) {
                    ...
                    _this8.io.store.addToHash("display:dc:" + _this8.name, "viewObjDisplayMap", JSON.stringify(map));
                    return vo;
                });
            } else {
        ...
        value: function onClosed(handler) {
            ...
            this.io.onTopic('display.displayContext.closed', function (msg, headers) {
                if (handler != null) {
                    var m = JSON.parse(msg.toString());
                    if (m.details.closedDisplayContext == _this9.name) handler(m, headers);
                }
            });
        }
    }, {
        key: 'onActivated',
        value: function onActivated(handler) {
            var _this10 = this;

            this.io.onTopic('display.displayContext.changed', function (msg, headers) {
                if (handler != null) {
                    var m = JSON.parse(msg.toString());
                    if (m.details.displayContext == _this10.name) handler(m, headers);
                }
            });
        }
    }, {
        key: 'onDeactivated',
        value: function onDeactivated(handler) {
            var _this11 = this;

            this.io.onTopic('display.displayContext.changed', function (msg, headers) {
                if (handler != null) {
                    var m = JSON.parse(msg.toString());
                    if (m.details.lastDisplayContext == _this11.name) handler(m, headers);
                }
            });
        }
    }]);
