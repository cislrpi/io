        ...
        value: function _generateEvents(pointer) {
            if (pointer) {
                ...
                if (!pointerState) {
                    ...
                    // smooth pointer x,y
                    var alpha = void 0;
                    var dt = pointer.details.time_captured - pointerState.lastTimeCaptured;
                    // v is mm/s
                    v = ((pointer.x - pointerState.lastX) * (pointer.x - pointerState.lastX) + (pointer.y - pointerState.lastY) * (pointer.y - pointerState.lastY)) * 1000 / dt;
                    if (v < 10) {
                        alpha = 2 * Math.PI * 100 / 9 * dt * 0.25;
                    }
                ...
                if (pointer.hit && Array.isArray(pointer.details.buttons)) {
                    ...
                    if (!pointerState.hardenedButtons) {
                        ...
                        try {
                            for (var _iterator3 = hardenedButtons[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
                                var b = _step3.value;
                        ...
                        try {
                            for (var _iterator4 = pointerState.hardenedButtons[Symbol.iterator](), _step4; !(_iteratorNormalCompletion4 = (_step4 = _iterator4.next()).done); _iteratorNormalCompletion4 = true) {
                                var _b = _step4.value;

                                if (!hardenedButtons.has(_b)) {
                                    pointerState.hardenedButtons.delete(_b);
                                    ...
                                    // within the region.
                                    if (pointerState.downButtons.has(_b)) {
                                        pointerState.downButtons.delete(_b);
