...
module.exports = function () {
    ...
    _createClass(CELIO, [{
        ...
        key: 'call',
        value: function call(queue, content, options) {
            var _this = this;
        ...
        key: 'onCall',
        value: function onCall(queue, handler) {
            var noAck = arguments.length <= 2 || arguments[2] === undefined ? true : arguments[2];

            if (this.pch) this.pch.then(function (ch) {
                ch.prefetch(1);
                ch.assertQueue(queue, { durable: false }).then(function (q) {
                    return ch.consume(q.queue, function (msg) {
                        var result = handler(msg.content, _.merge(msg.fields, msg.properties), function ack() {
                            ch.ack(msg);
                        });
