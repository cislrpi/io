            ...
            if (this.pch) this.pch.then(function (ch) {
                ...
                ch.assertQueue(queue, { exclusive: exclusive }).then(function (q) {
                    return ch.consume(q.queue, function (msg) {
                        var result = handler(msg.content, _.merge(msg.fields, msg.properties), noAck ? undefined : function ack() {
                            ch.ack(msg);
                        });

                        // If the result is an error then
                        if (result instanceof Error) {
                            ch.sendToQueue(msg.properties.replyTo, new Buffer(''), { correlationId: msg.properties.correlationId, headers: { error: result.message } });
                        } else {
            ...
            if (this.pch) this.pch.then(function (ch) {
                return ch.assertQueue('', { exclusive: true }).then(function (q) {
                    return ch.bindQueue(q.queue, _this2.exchange, topic).then(function () {
                        return ch.consume(q.queue, function (msg) {
                            return handler(msg.content, _.merge(msg.fields, msg.properties), { noAck: true });
                        });
                    });
                });
            });else throw new Error('Message exchange not configured.');
